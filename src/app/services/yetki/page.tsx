            ...
            <div className="space-y-12">
               ...
               <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-[0.4em] text-white/20">
                  <div className="flex items-center gap-2">
                     ...
                     <span className={step >= 2 ? 'text-amber-500' : ''}>TAŞINMAZ_VERİ</span>
                     <ChevronRight className="w-3 h-3" />
                     <span className={step >= 4 ? 'text-amber-500' : ''}>SAHİBİ_DAVET</span>
                     <ChevronRight className="w-3 h-3" />
                     <span className={step >= 6 ? 'text-primary' : ''}>YETKİ_MÜHÜR_OK</span>
                  </div>
               </div>

               {/* Step 1: Kod Erişimi */}
               {step === 1 && (
                 <div className="glass-card p-12 bg-amber-500/[0.02] border-amber-500/10 text-center space-y-10 animate-in fade-in duration-700 shadow-3xl">
                    <div className="w-20 h-20 rounded-[2.5rem] bg-amber-500/10 border border-amber-500/20 flex items-center justify-center mx-auto shadow-3xl shadow-amber-500/10 relative">
                       <Key className="w-10 h-10 text-amber-400" />
                    </div>
                    <div className="space-y-4">
                       <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">Dijital <br/> <span className="text-amber-500">Yetki Belgesi</span></h2>
                       <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] max-w-sm mx-auto">Resmi yetki bildirimini başlatmak için işlem kodunuzu girin.</p>
                    </div>
                    <div className="max-w-xs mx-auto space-y-6">
                       <input type="text" placeholder="TB-XXXX-X" className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-center text-xl font-black tracking-widest outline-none focus:border-amber-500/50" />
                       <button onClick={handleNext} className="w-full py-5 bg-amber-500 text-black font-black text-[12px] uppercase tracking-[0.3em] rounded-2xl hover:bg-amber-600 transition-all shadow-2xl">PROTOKOLÜ BAŞLAT</button>
                    </div>
                 </div>
               )}

               {/* Step 2: Taşınmaz Veri */}
               {step === 2 && (
