...
export default function IhalePage() {
  ...
  useEffect(() => {
    setMounted(true);
    // Kapı Kontrolü
    const gateStatus = localStorage.getItem('tb_gate_ihale');
  ...
  const onGateSuccess = () => {
    setGatePassed(true);
    localStorage.setItem('tb_gate_ihale', 'passed');
  };
  ...
  const clearSession = () => {
    if (confirm("Bu ihale oturumunu ve verileri sıfırlamak istediğinize emin misiniz?")) {
      ...
      localStorage.removeItem('tb_gate_ihale');
  ...
  return (
    <main className="min-h-screen bg-[#070709] selection:bg-sky-500/30 text-white">
      ...
      <div className="container mx-auto px-6 pt-32 pb-24">
        {!gatePassed ? (
          <ServiceGate serviceName="DİJİTAL İHALE VE ANALİZ" onSuccess={onGateSuccess} />
        ) : (
