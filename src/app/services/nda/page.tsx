...
export default function NDAPage() {
  ...
  useEffect(() => {
    setMounted(true);
    // Kapı Kontrolü
    const gateStatus = localStorage.getItem('tb_gate_nda');
  ...
  const onGateSuccess = () => {
    setGatePassed(true);
    localStorage.setItem('tb_gate_nda', 'passed');
  };
  ...
  const clearSession = () => {
    if (confirm("Bu sözleşme taslağını ve verileri silmek istediğinize emin misiniz?")) {
      ...
      localStorage.removeItem('tb_gate_nda');
  ...
  return (
    <main className="min-h-screen bg-[#070709] selection:bg-rose-500/30 text-white">
      ...
      <div className="container mx-auto px-6 pt-32 pb-24">
        {!gatePassed ? (
          <ServiceGate serviceName="HIZLI NDA (GİZLİLİK SÖZLEŞMESİ)" onSuccess={onGateSuccess} />
        ) : (
              ...
              <div className="space-y-12">
                ...
                <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-[0.4em] text-white/20">
                    <div className="flex items-center gap-2">
                      <span className={step >= 1 ? 'text-rose-400' : ''}>SÖZLEŞME_KONUSU</span>
                      <ChevronRight className="w-3 h-3" />
                      <span className={step >= 2 ? 'text-rose-400' : ''}>TARAF_BİLGİLERİ</span>
                      <ChevronRight className="w-3 h-3" />
                      <span className={step >= 3 ? 'text-rose-400' : ''}>MÜHÜR_OK</span>
                    </div>
                </div>

                {/* Step 1: Konu (Eski Step 2 Buraya Kaydı çünkü Gate Başta) */}
                {step === 1 && (
