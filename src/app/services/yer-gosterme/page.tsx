...
export default function YerGostermePage() {
  ...
  useEffect(() => {
    ...
    // Kapı Kontrolü
    const gateStatus = localStorage.getItem('tb_gate_yer_gosterme');
  ...
  const onGateSuccess = () => {
    setGatePassed(true);
    localStorage.setItem('tb_gate_yer_gosterme', 'passed');
  };
  ...
  const clearSession = () => {
    if (confirm("Bu işlem mühürlerini ve verileri silmek istediğinize emin misiniz?")) {
      ...
      localStorage.removeItem('tb_gate_yer_gosterme');
  ...
  return (
    <main className="min-h-screen bg-[#070709] selection:bg-primary/30 text-white">
      ...
      <div className="container mx-auto px-6 pt-32 pb-24">
        {!gatePassed ? (
          <ServiceGate serviceName="DİJİTAL YER GÖSTERME" onSuccess={onGateSuccess} />
        ) : (
              ...
              <div className="space-y-12">
                ...
                <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-[0.4em] text-white/20">
                    <div className="flex items-center gap-2">
                      <span className={step >= 1 ? 'text-amber-500' : ''}>EMLAKÇI_KİMLİK</span>
                      <ChevronRight className="w-3 h-3" />
                      <span className={step >= 3 ? 'text-amber-500' : ''}>MÜŞTERİ_TEYİT</span>
                      <ChevronRight className="w-3 h-3" />
                      <span className={step >= 4 ? 'text-amber-500' : ''}>RESMİ_MÜHÜR_OK</span>
                    </div>
                </div>

                {/* Step 1: Emlakçı Bilgileri (Eski Step 2 Buraya Kaydı çünkü Gate Başta) */}
                {step === 1 && (
