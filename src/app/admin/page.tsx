...
export default function AdminDashboard() {
  ...
  const [activeCodes, setActiveCodes] = useState(8);
  const [expiredCodes, setExpiredCodes] = useState(45);
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          <button className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-xs font-bold hover:bg-white/10 transition-all uppercase tracking-widest">
            <RefreshCcw className="w-3 h-3" /> Verileri Yenile
          </button>
          <button className="btn-primary py-2 px-4 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="glass-card overflow-hidden">
            ...
            <div className="divide-y divide-white/5">
              {[
                ...
                { user: "admin@trust.com", code: "TB-AD01-A", time: "21:05:44", status: "Aktif", payment: "Shopier" },
                { user: "test@user.xyz", code: "TB-ZZ99-P", time: "18:22:10", status: "Kullanıldı", payment: "Shopier" },
              ].map((row, idx) => (
                <div key={idx} className="p-4 flex items-center justify-between hover:bg-white/[0.01] transition-colors group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-sm font-bold">{row.user}</div>
                      <div className="text-[10px] font-black text-primary tracking-widest uppercase">{row.code}</div>
                    </div>
                  </div>
                  <div className="text-right flex items-center gap-8">
                    <div>
                      <div className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Kalan Süre</div>
        ...
        <div className="space-y-6">
          <div className="glass-card p-6 bg-primary/5 border-primary/20">
            ...
            <div className="space-y-6">
              {[
                { label: "Firebase Database", value: "Açık", color: "text-emerald-400" },
                { label: "Shopier Webhook", value: "Beklemede", color: "text-amber-400" },
                { label: "RAM-Only Buffer", value: "Temiz", color: "text-emerald-400" },
              ].map((item, idx) => (
